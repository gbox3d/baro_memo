...
**팀 — 접근 격리**(0.13.0). 글마다 팀이 하나 있고, 자기가 속한 팀의 글만 보입니다. 분류가
아니라 **기밀**입니다 — 기본은 여전히 보드 하나(`team-n`, 전원이 암묵 소속)이고, 팀은 비밀
프로젝트를 일반 구성원과 격리하는 예외를 위해 있습니다. 내장 둘: `team-n`(기본팀)과
`super`(전 팀 보기·쓰기 — 대표·이사진 층). 팀 생성과 소속 관리는 관리자 페이지에서, 소속은
토큰이 아니라 **사람**에 걸립니다(재발급해도 그대로). 밖에서 보이지 않는 팀은 글도 팀 이름도
404 입니다 — 비밀 프로젝트는 존재 자체가 정보라서, 403 으로 "있긴 하다"를 말해 주지 않습니다.
