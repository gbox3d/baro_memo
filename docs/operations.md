...
```bash
sqlite3 /mnt/data/baro_memo_db/memo.db     ".backup '/백업/memo-$(date +%F).db'"
sqlite3 /mnt/data/baro_memo_files/files.db ".backup '/백업/files-$(date +%F).db'"
rsync -a /mnt/data/baro_memo_files/store/ /백업/store/
install -m600 /mnt/data/baro_memo_db/admin-token /백업/admin-token
```

`sqlite3` 가 없는 호스트라면 Node 로도 같은 일을 합니다(새로 깔 것 없이):

```bash
node -e "new (require('node:sqlite').DatabaseSync)('<원본>',{readOnly:true}).exec(\"VACUUM INTO '<사본>'\")"
```

확인: 사본을 열어 건수를 맞춰 봅니다 — `sqlite3 <사본> "select count(*) from memo;"` 가
`/memo/api/health` 의 `board.total` 과 같아야 합니다.
