...
- 현재 버전: 0.13.0 — **팀 접근 격리**. 글마다 팀이 하나 있고 자기 팀만 보인다(안 보이는 글은
  없는 글과 같은 404). 기본은 여전히 보드 하나 — 전원이 `team-n` 암묵 소속이고, 기존 글은
  전부 거기로 이관됐다. 앞선 축: 읽기도 토큰(0.5.0), 이력이 두 층으로(0.6.0/0.7.0),
  중요도 점수(0.8.0), `/api/auth/whoami`(0.9.0), 저장소를 마운트로 합침(0.11.0),
  영어 집행을 걷어내고 문서로 되돌림(0.12.0)
...
- 검사: `pnpm test` 227개 (백엔드 145 + 관리자 페이지 45 + 아티팩트 저장소 37)
